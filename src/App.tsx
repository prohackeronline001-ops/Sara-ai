...
import { getSaraResponse, getSaraAudio, resetSaraSession, VOICES, setSaraVoice } from "./services/geminiService";
...
export default function App() {
  ...
  const [selectedVoice, setSelectedVoice] = useState<string>(() => {
    const saved = localStorage.getItem("sara_selected_voice");
    return saved || "Aishwarya";
  });

  useEffect(() => {
    localStorage.setItem("sara_selected_voice", selectedVoice);
    setSaraVoice(selectedVoice);
    if (liveSessionRef.current) {
      liveSessionRef.current.voiceName = VOICES[selectedVoice as keyof typeof VOICES];
    }
  }, [selectedVoice]);
  ...
  const toggleListening = async () => {
    if (isSessionActive) {
      ...
      try {
        ...
        session.voiceName = VOICES[selectedVoice as keyof typeof VOICES];
      ...
      <header className="absolute top-0 left-0 w-full flex justify-between items-center z-20 shrink-0 px-6 py-4 md:px-12 md:py-6">
        ...
        <div className="flex items-center gap-2">
          <select 
            ...
            className="bg-white/5 border border-white/10 rounded-full px-3 py-1.5 text-xs text-white/70 outline-none focus:border-violet-500/50 transition-colors cursor-pointer hover:bg-white/10"
          >
            {Object.keys(VOICES).map(voice => (
              <option key={voice} value={voice} className="bg-[#050505] text-white">
                {voice}
              </option>
            ))}
          </select>
